...
public class UtilTime
{
    ...
    public static String convertTime(long time) {
        ...
        String build = "";
        if (day > 0L) {
            build = String.valueOf(build) + day + " day" + ((day == 1L) ? " " : "s ");
        }
        if (hour > 0L) {
            build = String.valueOf(build) + hour + " hour" + ((hour == 1L) ? " " : "s ");
        }
        if (minute > 0L) {
            build = String.valueOf(build) + minute + " minute" + ((minute == 1L) ? " " : "s ");
        }
        if (second > 0L) {
            build = String.valueOf(build) + second + " second" + ((second == 1L) ? " " : "s ");
        }
        if (build.length() == 0) {
            build = "0 seconds";
        }
