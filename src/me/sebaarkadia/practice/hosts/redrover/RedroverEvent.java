package me.sebaarkadia.practice.hosts.redrover;
...
public class RedroverEvent extends PracticeEvent<RedroverPlayer> {
    ...
    public List<CustomLocation> getSpawnLocations() {
        return Collections.singletonList(this.getPlugin().getManagerHandler().getSpawnManager().getRedroverLocation());
    }
            ...
            if(data != null) {
                if(data.getState() == RedroverPlayer.RedroverState.FIGHTING || data.getState() == RedroverPlayer.RedroverState.PREPARING) {
                    ...
                    this.getPlugin().getServer().getScheduler().runTask(this.getPlugin(), () -> {
                        this.getPlugin().getManagerHandler().getPracticePlayerManager().sendToLobby(player);;
                        if(this.getPlayers().size() >= 2) {
                            this.getPlugin().getManagerHandler().getEventManager().addSpectatorRedrover(player, this.getPlugin().getManagerHandler().getPracticePlayerManager().getPracticePlayer(player), this);
                        }
    ...
    private CustomLocation[] getGameLocations() {
        final CustomLocation[] array = {this.getPlugin().getManagerHandler().getSpawnManager().getRedroverFirst(), this.getPlugin().getManagerHandler().getSpawnManager().getRedroverSecond()};
        return array;
    }
            ...
            public void run() {
                ...
                for (final Player player : array) {
                    if (RedroverEvent.this.streakPlayer == null || RedroverEvent.this.streakPlayer != player.getUniqueId()) {
                        ...
                        RedroverEvent.this.getPlugin().getManagerHandler().getKitManager().getKit("NoDebuff").applyToPlayer(player);
