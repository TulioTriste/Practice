package me.sebaarkadia.practice.hosts;
...
public abstract class PracticeEvent<K extends EventPlayer> {
    ...
    public void join(final Player player) {
        ...
        final PracticePlayer playerData = this.plugin.getManagerHandler().getPracticePlayerManager().getPracticePlayer(player);
        playerData.setCurrentState(PlayerState.EVENT);
        ...
        this.plugin.getManagerHandler().getPracticePlayerManager().getPracticePlayer(player).giveLobbyItems(player);
    ...
    public void start() {
        ...
        this.onStart();
        this.plugin.getManagerHandler().getEventManager().setCooldown(0L);
    }

    public void end() {
        this.plugin.getServer().getScheduler().runTaskLater(this.plugin, () -> this.plugin.getManagerHandler().getEventManager().getEventWorld().getPlayers().forEach(player -> this.plugin.getManagerHandler().getPracticePlayerManager().sendToLobby(player)), 2L);
        this.plugin.getManagerHandler().getEventManager().setCooldown(System.currentTimeMillis() + 300000L);
        ...
        final Iterator<UUID> iterator = this.plugin.getManagerHandler().getEventManager().getSpectators().keySet().iterator();
        while (iterator.hasNext()) {
            final UUID spectatorUUID = iterator.next();
            final Player spectator = Bukkit.getPlayer(spectatorUUID);
            if (spectator != null) {
                this.plugin.getServer().getScheduler().runTask(this.plugin, () -> this.plugin.getManagerHandler().getPracticePlayerManager().getPracticePlayer(spectator).setCurrentState(PlayerState.LOBBY));
                iterator.remove();
            }
        }
        this.plugin.getManagerHandler().getEventManager().getSpectators().clear();
