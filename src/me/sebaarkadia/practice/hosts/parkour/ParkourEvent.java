...
public class ParkourEvent extends PracticeEvent<ParkourPlayer> {
    ...
    public List<CustomLocation> getSpawnLocations() {
        return Collections.singletonList(this.getPlugin().getManagerHandler().getSpawnManager().getParkourLocation());
    }
    ...
    public Consumer<Player> onDeath() {
        return player -> {
            ...
            PracticePlayer playerData = Practice.getInstance().getManagerHandler().getPracticePlayerManager().getPracticePlayer(player);
    ...
    private void teleportToSpawnOrCheckpoint(final Player player) {
        ...
        player.teleport(this.getPlugin().getManagerHandler().getSpawnManager().getParkourGameLocation().toBukkitLocation());
    }

    private void giveItems(final Player player) {
        this.getPlugin().getServer().getScheduler().runTask(this.getPlugin(), () -> {
            PlayerUtil.clearPlayer(player);
            player.getInventory().setItem(0, UtilItem.createItem(Material.FIREBALL, ChatColor.GREEN.toString() + ChatColor.BOLD + "Toggle Visibility"));
            player.getInventory().setItem(4, UtilItem.createItem(Material.NETHER_STAR, ChatColor.RED.toString() + ChatColor.BOLD + "Leave Event"));
