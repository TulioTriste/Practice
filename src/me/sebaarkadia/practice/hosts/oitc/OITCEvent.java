...
public class OITCEvent extends PracticeEvent<OITCPlayer> {
    ...
    public List<CustomLocation> getSpawnLocations() {
        return Collections.singletonList(this.getPlugin().getManagerHandler().getSpawnManager().getOitcLocation());
    }
                ...
                if(data.getState() != OITCPlayer.OITCState.WAITING) {
                    if(data.getState() == OITCPlayer.OITCState.FIGHTING || data.getState() == OITCPlayer.OITCState.PREPARING || data.getState() == OITCPlayer.OITCState.RESPAWNING) {
                        ...
                        if(data.getLastKiller() != null) {
                            ...
                            respawnTask.getInventory().setItem(6, UtilItem.createItem(Material.GLOWSTONE_DUST, ChatColor.YELLOW.toString() + ChatColor.BOLD + "Kills", playerData.getScore() == 0?1:playerData.getScore()));
                            ...
                            PracticePlayer playerData1 = this.getPlugin().getManagerHandler().getPracticePlayerManager().getPracticePlayer(respawnTask);
                            ...
                            if(count == 25) {
                                PracticePlayer winnerData = Practice.getInstance().getManagerHandler().getPracticePlayerManager().getPracticePlayer(respawnTask);
                        ...
                        if(data.getLastKiller() == null) {
                            ...
                            PracticePlayer var12 = this.getPlugin().getManagerHandler().getPracticePlayerManager().getPracticePlayer(player);
                            ...
                            if(data.getLives() == 0) {
                                ...
                                this.getPlugin().getServer().getScheduler().runTask(this.getPlugin(), () -> {
                                    this.getPlugin().getManagerHandler().getPracticePlayerManager().getPracticePlayer(player.getUniqueId()).setCurrentState(PlayerState.LOBBY);
                                    if(this.getPlayers().size() >= 2) {
                                        this.getPlugin().getManagerHandler().getEventManager().addSpectatorOITC(player, this.getPlugin().getManagerHandler().getPracticePlayerManager().getPracticePlayer(player), this);
                                    }
    ...
    private List<CustomLocation> getGameLocations() {
        if (this.respawnLocations != null && this.respawnLocations.size() == 0) {
            this.respawnLocations.addAll(this.getPlugin().getManagerHandler().getSpawnManager().getOitcSpawnpoints());
        }
    ...
    private void giveRespawnItems(final Player player, final OITCPlayer oitcPlayer) {
        this.getPlugin().getServer().getScheduler().runTask(this.getPlugin(), () -> {
            ...
            player.getInventory().setItem(0, UtilItem.createItem(Material.WOOD_SWORD, ChatColor.GREEN + "Wood Sword"));
            player.getInventory().setItem(1, UtilItem.createItem(Material.BOW, ChatColor.GREEN + "Bow"));
            player.getInventory().setItem(6, UtilItem.createItem(Material.GLOWSTONE_DUST, ChatColor.YELLOW.toString() + ChatColor.BOLD + "Kills", (oitcPlayer.getScore() == 0) ? 1 : oitcPlayer.getScore()));
            player.getInventory().setItem(7, UtilItem.createItem(Material.REDSTONE, ChatColor.RED.toString() + ChatColor.BOLD + "Lives", oitcPlayer.getLives()));
    ...
    public class OITCGameTask extends BukkitRunnable {
        ...
        public void run() {
            if (this.time == 303) {
                ...
                if (winner != null) {
                    final PracticePlayer winnerData = Practice.getInstance().getManagerHandler().getPracticePlayerManager().getPracticePlayer(winner);
                    winnerData.setOitcEventWins(winnerData.getOitcEventWins() + 1);
                    for (int i = 0; i <= 2; ++i) {
