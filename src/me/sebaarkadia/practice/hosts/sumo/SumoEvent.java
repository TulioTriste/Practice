...
public class SumoEvent extends PracticeEvent<SumoPlayer> {
    ...
    public List<CustomLocation> getSpawnLocations() {
        return Collections.singletonList(this.getPlugin().getManagerHandler().getSpawnManager().getSumoLocation());
    }
        ...
        return player -> {
            ...
            if (data != null && data.getFighting() != null) {
                if (data.getState() == SumoPlayer.SumoState.FIGHTING || data.getState() == SumoPlayer.SumoState.PREPARING) {
                    ...
                    PracticePlayer playerData = this.getPlugin().getManagerHandler().getPracticePlayerManager().getPracticePlayer(player);
                    if (playerData != null) {
                    ...
                    this.getPlugin().getManagerHandler().getPracticePlayerManager().getPracticePlayer(player).giveLobbyItems(player);
                    PlayerUtil.clearPlayer(killer);
                    this.getPlugin().getManagerHandler().getPracticePlayerManager().getPracticePlayer(player).giveLobbyItems(killer);
                    ...
                    if (this.getByState(SumoPlayer.SumoState.WAITING).size() == 1) {
                        ...
                        PracticePlayer winnerData = Practice.getInstance().getManagerHandler().getPracticePlayerManager().getPracticePlayer(winner);
    ...
    private CustomLocation[] getSumoLocations() {
        final CustomLocation[] array = {this.getPlugin().getManagerHandler().getSpawnManager().getSumoFirst(), this.getPlugin().getManagerHandler().getSpawnManager().getSumoSecond()};
        return array;
    }
    ...
    private void selectPlayers() {
        ...
        for (final UUID spectatorUUID : this.getPlugin().getManagerHandler().getEventManager().getSpectators().keySet()) {
